package 
{
	...
	public class Bishop extends Sprite
	{
		...
		public var baseDamage:int = 40;
		public var totalDamage:int = 40;
		public var expYield:int = 40;
		public var armor:int = 0;
		public var reflexes:int;
		public var lethality:int;
		public var protection:int = 0
		public var disappeared:Boolean = false
		...
		public function Bishop(color:String, equipment:Array, skills:Array, lvl:int, exp:int, startingPosition:String)
		{
			this.startingPosition = startingPosition
			This = this
			this.skills = skills;
			this.team = color;
			this.totalHealth = 80 + ((lvl - 1) * 20);
			this.currentHealth = this.totalHealth;
			this.currentExp = exp;
			this.totalExp = 100 + (lvl * 25);
			this.currentMana = 100;
			this.lvl = lvl;
			model = new (getDefinitionByName(color + "Bishop") as Class);
			...
			{
				if (skills[i] == "reflexes")
				{
					reflexes = skills[i + 1];
					dodgeChance += reflexes * 0.01
				}
				if (skills[i] == "lethality")
				{
					lethality = skills[i + 1];
					instantKillChance += lethality * 0.005
				}
			}
		}
		...
		public function throwStar(xDistance:int, yDistance:int):void
		{
			Animations.throwStar(This,inFrontContainer,inBackContainer,xDistance,yDistance)
		}
		public function disappear():void
		{
			Animations.disappear(This,inFrontContainer,inBackContainer)
		}
		public function assassinate(xDistance:int, yDistance:int):void
		{
			Animations.assassinate(This,inFrontContainer,inBackContainer,xDistance,yDistance)
		}
		public function startAttack(currentSet:String, playerStart:Boolean, sideLength:int):void
		{
			...
			if ((playerStart && team == "White") || (!playerStart && team == "Black"))
			{
				frontBack = "Back";
			}
			else
			...
			model = new (getDefinitionByName(team + currentSet + frontBack + "Bishop") as Class);
		...
		{
			removeChild(model)
			model = new (getDefinitionByName(team + "Bishop") as Class);
			addChild(model);
			addChild(healthBar);
			addChild(manaBar);
			addChild(xpBar);
