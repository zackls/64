package
{
	...
	public class Pawn extends Sprite
	{
		...
		public function Pawn(color:String, equipment:Array, skills:Array, lvl:int, exp:int, startingPosition:String)
		{
			...
			this.lvl = lvl
			model = new (getDefinitionByName(color + "Pawn") as Class)
			...
			for (i = 0; i < skills.length; i++)
			{
				...
				{
					defense = skills[i + 1]
					armor += Math.round(armor * 0.03 * defense)
				}
		...
		{
			Animations.defend(This,inFrontContainer,inBackContainer)
		}
		public function startAttack(currentSet:String, playerStart:Boolean, sideLength:int):void
		{
			...
			model = new (getDefinitionByName(team + currentSet + "Pawn") as Class)
			addChild(inBackContainer)
			addChild(model);
			addChild(inFrontContainer)
			model.height = 450 / sideLength
			model.width = 100
			model.rotationX = 90
		}
		...
		{
			removeChild(model)
			model = new (getDefinitionByName(team + "Pawn") as Class);
			addChild(model);
			addChild(healthBar);
			addChild(manaBar);
			addChild(xpBar);
