package
{
	...
	public class Knight extends Sprite
	{
		...
		public var baseDamage:int = 60
		public var totalDamage:int = 60
		public var expYield:int = 40
		public var armor:int = 0
		public var channeling:int
		...
		public function Knight(color:String, equipment:Array, skills:Array, lvl:int, exp:int, startingPosition:String)
		{
			...
			this.skills = skills
			this.team = color
			this.totalHealth = 100 + ((lvl - 1) * 25)
			this.currentHealth = this.totalHealth
			this.currentExp = exp
			this.totalExp = 100 + (lvl * 25)
			this.currentMana = 100
			this.lvl = lvl
			model = new (getDefinitionByName(color + "Knight") as Class)
			...
			{
				if (skills[i] == "channeling")
				{
					channeling = skills[i + 1]
					manaLeech += channeling
				}
				if (skills[i] == "critical")
				{
					critical = skills[i + 1]
					criticalChance += 0.01 * critical
				}
			}
		}
		...
		public function longshot(target:Object):void
		{
			Animations.longshot(This,inFrontContainer,inBackContainer,target)
		}
		public function aimedShot(xDistance:int, yDistance:int)
		{
			Animations.aimedShot(This,inFrontContainer,inBackContainer,xDistance,yDistance)
		}
		public function startAttack(currentSet:String, playerStart:Boolean, sideLength:int):void
		{
			...
			model = new (getDefinitionByName(team + currentSet + frontBack + "Knight") as Class)
		...
		{
			removeChild(model)
			model = new (getDefinitionByName(team + "Knight") as Class);
			addChild(model);
			addChild(healthBar);
			addChild(manaBar);
			addChild(xpBar);
