package
{
	...
	public class Queen extends Sprite
	{
		...
		public function Queen(color:String, equipment:Array, skills:Array, lvl:int, exp:int, startingPosition:String)
		{
			...
			this.lvl = lvl
			model = new (getDefinitionByName(color + "Queen") as Class)
			...
			{
				if (skills[i] == "vitality")
				{
					vitality = skills[i + 1]
					healthRegen += vitality / 100
				}
				if (skills[i] == "focus")
				{
					focus = skills[i + 1]
		...
		public function throwFireball(xDistance:int,yDistance:int):void
		{
			Animations.throwFireball(This,inFrontContainer,inBackContainer,xDistance,yDistance)
		}
		...
		public function shootLaser(xDistance,yDistance)
		{
			Animations.shootLaser(This,inFrontContainer,inBackContainer,xDistance,yDistance)
		}
		public function shield():void
		{
			Animations.shield(This,inFrontContainer,inBackContainer)
		}
		public function startAttack(currentSet:String, playerStart:Boolean, sideLength:int):void
		{
			...
			model = new (getDefinitionByName(team + currentSet + "Queen") as Class)
		...
		{
			removeChild(model)
			model = new (getDefinitionByName(team + "Queen") as Class);
			addChild(model);
			addChild(healthBar);
			addChild(manaBar);
			addChild(xpBar);
